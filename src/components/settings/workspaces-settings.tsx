...
export function WorkspacesSettings() {
  ...
  const handleBulkDelete = async () => {
    ...
    if (!confirm(`Are you sure you want to delete ${ownedSelected.length} workspace(s)? This action cannot be undone.`)) return;
  ...
  const handleBulkLeave = async () => {
    ...
    if (!confirm(`Are you sure you want to leave ${joinedSelected.length} workspace(s)?`)) return;
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto w-full relative h-full flex flex-col">
      <div className="mb-6 flex-shrink-0">
        <h1 className="text-2xl font-bold">My Workspaces</h1>
        <p className="text-muted-foreground mt-1">Manage workspaces you own or have joined.</p>
      </div>
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-muted/50 border-b border-border/50 text-muted-foreground">
                <tr>
                  ...
                  <th className="p-4 font-medium">Workspace Name</th>
                ...
                {workspaces.map(workspace => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 font-medium flex items-center gap-2 max-w-[150px] sm:max-w-[300px] md:max-w-[400px]">
      ...
      {selectedIds.length > 0 && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-popover border shadow-lg rounded-full px-6 py-3 flex items-center gap-4 animate-in slide-in-from-bottom-5">
          ...
          {joinedSelected.length > 0 && (
            <Button variant="outline" size="sm" onClick={handleBulkLeave} disabled={isLeaving}>
